    ...
    <div class="mb-4">
        ...
        <form id="filterForm" method="GET" action="{{ route('pembayaran-penjualan.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            ...
            <div>
                ...
                <input type="text" name="notajual" value="{{ request('notajual') }}" class="w-full border rounded px-2 py-1" placeholder="Opsional">
            </div>
                    ...
                    content.innerHTML = data.map(item => `
                        <div class="border-b pb-2 bg-white-100 p-3 rounded mb-2 shadow-sm">
                            ...
                            <div class="mt-2 flex gap-2">
                                <button onclick="editBayar('${item.no}')" class="bg-blue-100 text-blue-700 px-3 py-1 rounded hover:bg-blue-200 text-sm">Edit</button>
                                <button onclick="hapusBayar('${item.no}')" class="bg-red-100 text-red-700 px-3 py-1 rounded hover:bg-red-200 text-sm">Hapus</button>
                            </div>
                        </div>
                    `).join('');
                }
